...
void FSTGPU::IO_setup( TP* cpu_transducer, uint32_t blocks, uint32_t threads){
  ...
  for ( uint32_t i = 0; i < inputCount; i++){
    ...
    error = cudaMemcpy ( (temp_input[i]), (cpu_transducer->input[i]),cpu_transducer->input_length[i] * sizeof(uint32_t),  cudaMemcpyHostToDevice);
...
#endif
  error = cudaMalloc( & (printed_output), 200* sizeof( uint32_t)); 
  printf( "Allocate dumbmy output  assert %s \n", cudaGetErrorString(error));                               
  error = cudaMemset( printed_output,0,  200* sizeof( uint32_t)); 
  printf( "Set dumbmy output  assert %s \n", cudaGetErrorString(error));  
  printf (" =====================================================\n");
}
