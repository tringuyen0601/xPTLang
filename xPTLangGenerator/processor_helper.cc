...
void TP::readTransition ( std::string filename , int verbose ){
  ...
  while ( getline(fin, line)){
    ...
    /* input Tape Condition  */
    char * InputTapeCompare = strtok( NULL, ": , - |");
    std::string str_InputTapeCompare = std::string(InputTapeCompare);
    if( verbose) std::cout << str_InputTapeCompare  << "-";
    transitionList[i].IcompareCode  = (uint32_t) std::stoi(str_InputTapeCompare);
    ...
    /* Stack  Condition  */
    char * stackMatchCompare = strtok( NULL, ": , - |");
    std::string str_stackMatchCompare = std::string(stackMatchCompare);
    if( verbose) std::cout << str_stackMatchCompare  << "-";
    transitionList[i].ScompareCode  = (uint32_t) std::stoi(str_stackMatchCompare);
  ...
  for ( uint32_t i=0; i<stateCount; i++ ){
    ...
    for ( uint32_t j =0; j < transitionCount; j++){
      if ( transitionList[j].currentState == i){
        ...
        reorder[count].IcompareCode = transitionList[j].IcompareCode ;
        reorder[count].inputSymbol  = transitionList[j].inputSymbol;
        reorder[count].IvarID  = transitionList[j].IvarID;
        reorder[count].ScompareCode = transitionList[j].ScompareCode ;
...
//// create states////
  ...
  for ( uint32_t i = 0; i < stateCount; i++){
    ...
    if( ( stateList[i].accepting == true) && (stateList[i].initial == false ) ){
...
std::string TP::compare_code_convert( uint32_t code){
  std::string r_code = "!";
  switch ( code) {
    case 0:
      r_code = "==";
...
void TP::printTransitionTable(){
  for ( uint32_t i =0; i < transitionCount; i++){
    ...
    std::cout << compare_code_convert( transitionList[i].IcompareCode) ;
    std::cout << transition_convert( transitionList[i].inputSymbol,1) << ", ";
    std::cout << transition_convert( transitionList[i].IvarID,1) ;
    std::cout << compare_code_convert( transitionList[i].ScompareCode) ;
