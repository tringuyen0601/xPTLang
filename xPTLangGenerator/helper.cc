...
helper::helper ( std::string * infile, std::string  * outfile, uint32_t inputsize,  int test){
  ...
  switch(test){
    ...
    case CSV_ENC_DET:
      ...
      break;
    case CSV_ENC_INDET:
      base            = "./testcases/csv_parsing_indet";
      stateFile       = base + "/csv_state.txt";
      transitionFile  = base + "/csv_transition.txt";
      configFile      = base + "/csv_config.txt";
      dotFile         = "./graph/csv_enc_indet_";
      break;
    case  HISTOGRAM:
      base            = "./testcases/histogram";
      stateFile       = base + "/hist_state.txt";
      transitionFile  = base + "/hist_transition.txt";
      configFile      = base + "/hist_config.txt";
      dotFile         = "./graph/hist_novar_";
      break;
  }
...
void helper::transducer_config(){
  ...
  token = strtok( NULL, ", |");
  str_token = std::string(token);
  bitmap = uint64_t (std::stoi(str_token));
  std::cout << " Stack bitmap: " << bitmap <<std::endl;
}
