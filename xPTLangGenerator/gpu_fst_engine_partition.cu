...
std::cout <<"=======================================" << std::endl;
  ...
  switch(test){
    ...
    case CSV_ENC_DET:
      ...
      destination = destination +  "csr_parsing/gpu_thread_config";
      CSV_PARSING( cpu_transducer);
      break;
    case CSV_ENC_INDET:
      std::cout << " CSV_ENC_INDET" << std::endl;
      destination = destination +  "csr_parsing_indet/gpu_thread_config";
      CSV_PARSING( cpu_transducer);
      break;
    case HISTOGRAM:
      std::cout << "HISTOGRAM" << std::endl;
      destination = destination +  "histogram/gpu_thread_config";
      RLE_ENCODING( cpu_transducer);
      break;
  ...
  for ( uint32_t i = 0; i < total_threads; i++){
    ...
    for (uint32_t j = 0; j < cpu_transducer->inputCount; j++){
      ...
      for ( uint32_t k = partition_input_base_cpu[i][j] ; k < partition_input_base_cpu[i][j] + partition_input_length_cpu[i][j] ; k++)
        pconf << cpu_transducer->input[j][k] << " ";
      pconf << std::endl;
      */
      pconf << "--------------------" << std::endl;
    }
...
void FSTGPU::COOCSR( TP * cpu_transducer){
  ...
  uint32_t number_of_data = cpu_transducer->input[0][2]; 
  uint32_t number_of_row = cpu_transducer->input[0][0]; 
  uint32_t number_of_col = cpu_transducer->input[0][1]; 
...
void FSTGPU::CSV_PARSING( TP * cpu_transducer){
  std::cout << "==================================== "<< std::endl;
  std::cout << " START PARTITION INPUT FOR CSV PARSING"<< std::endl;
  uint32_t thread_length = cpu_transducer->input_length[0] / total_threads;
